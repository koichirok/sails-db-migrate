...
module.exports = function (grunt) {
  grunt.registerTask('db:migrate', 'Run the database migrations', function (command) {
    ...
    // config.
    sails = new Sails();
    // load config from sailsrc and override some of them with task's default
    sailsConfig = sails.getRc();
    _.merge(sailsConfig, {
      port: -1,
      log: { level: process.env.LOG_LEVEL || 'silent' },
      environment: grunt.option('env') || sailsConfig.environment || process.env.NODE_ENV,
      migrating: true
    })
